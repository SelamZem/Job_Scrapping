...
class RSSWorkingNomadsScraper(BaseScraper):
    ...
    def scrape_jobs(self, search_query: str, location: str = "", max_pages: int = 2) -> List[Dict]:
        ...
        try:
            ...
            print(f"Fetching jobs from Working Nomads RSS feed")
            ...
            if feed.entries:
                print(f"Found {len(feed.entries)} jobs in Working Nomads RSS feed")
            ...
            print(f"Error fetching from Working Nomads RSS: {e}")
        
        print(f"Total jobs from Working Nomads RSS: {len(jobs)}")
        return jobs
